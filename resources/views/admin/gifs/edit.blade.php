 @extends('admin.layouts.master')

@section('content')
	<div class="row body">
		<div class="col-sm-12 col-md-12">
			<div class="panel work-wrapper">
				<div class="panel-heading"><h3>@yield('title', 'Editar Gif')</h3></div>
				  <div class="panel-body">
				  	{{ Form::open(['route' => ['gifs.update',$gif->id],"method"=>"PUT",'files'=>true,'id'=>'gif']) }}
				  	<div class="form-group">
				  		{{Form::label('title','Titulo')}}
						{{Form::text('title',$gif->title, ['class'=>'form-control','placeholder'=>'Título del Gif','required','accept'=>'image/gif'] ) }}
				  	</div>

				  	<div class="form-group">
				  		<img class="img-responsive" src="{{url('gifs/'.$gif->gif)}}">
				  	</div>

				  	<div class="form-group">
				  		{{Form::label('description','Descripción')}}
						{{Form::textarea('description',$gif->description, ['class'=>'form-control editor','placeholder'=>'Descripción','required'] ) }}
				  	</div>

				  	<div class="form-group  ">
				  	<a class="btn btn-default pull-left" href="{{route('gifs.index')}}"><i class="fa fa-undo"></i>  Regresar</a>
				
				  	{{Form::submit('Actualizar',['class'=>'btn btn-info pull-right']) }}
				  	</div>
				  	{{ Form::close() }}
				  </div>
				</div>
			</div>
		</div>
@endsection